        ...
        for item_code, net_amount in self.invoice_items.get(invoice).items():
            if item_code in items:
                if self.item_tax_rate.get(
                    ...
                    taxable_value += abs(net_amount)
    ###################################### Date : May 11,2023
    # Customize Line Adding CGST/SGST/IGST
        if division_factor == 2:
            cgst = flt((taxable_value * (tax_rate/division_factor)) / 100.0, 2)
            sgst = flt((taxable_value * (tax_rate/division_factor)) / 100.0, 2)
        else:
            cgst = 0
            sgst = 0
        if division_factor == 1:
            igst = flt((taxable_value * (tax_rate/division_factor)) / 100.0, 2)
        else:
            igst = 0
    ######################################
        row += [tax_rate or 0, cgst, sgst, igst, taxable_value]
    ...
    def get_columns(self):
        ...
        if self.filters.get("type_of_business") != "NIL Rated":
            self.tax_columns = [
                ...
                {"fieldname": "cgst", "label": "CGST", "fieldtype": "Currency", "width": 100},
                {"fieldname": "sgst", "label": "SGST", "fieldtype": "Currency", "width": 100},
                {"fieldname": "igst", "label": "IGST", "fieldtype": "Currency", "width": 100},
                {
